...
- Migrations : `supabase/migrations/0001..0074` (dernière appliquée : **0074** —
  correctif RLS isolation multi-site : droppe les policies permissives orphelines
  `stp_select`/`stp_modify` (`semaine_type_profil`, 0028) et `pcr_select`/`pcr_modify`
  (`poste_competence_requise`, 0032) que la 0043 n'avait jamais droppées, et qui
  rendaient ces deux tables visibles cross-site (semaines type d'un site affichées
  sur un autre).
  Détail 0073 —
