...
- **`poste_quart` = effectif PAR quart** (migration 0070, helper unique
  `src/lib/poste-quart.ts`). ⚠️ L'ancien invariant « ne stocke que les désactivations,
  défaut actif » N'EST PLUS VRAI. La table porte l'effectif de chaque (poste × quart)
  via `effectif_requis` (int nullable), lu en **trois états** par `etatQuart` :
  **aucune ligne** = repli sur `poste.effectif_requis` (colonne **DÉPRÉCIÉE**, gardée
  comme repli/source de reprise, plus la source de vérité) ; **`actif=false`** = « – »
  (le poste **ne tourne pas** sur ce quart) ; **`actif=true`** = tourne, effectif **0**
  (« tourne à 0 », distinct de « – » : le poste tourne mais 0 requis) ou **N**.
  Helpers : `effectifSurQuart(pq, posteId, quart, posteEff)` (0 si ne tourne pas),
  `tourneSurQuart(...)`. Chargement `chargerPosteQuart(supabase)` (fetchAll, tolère
  l'absence de la colonne). Le Référentiel saisit un **champ numérique par quart**
  (vide « – » / 0 / N), la colonne « Effectif » unique a disparu.
- **Ouverture / fermeture datée** (`ligne`/`poste`, migration 0071, helper
  `src/lib/referentiel-validite.ts`). Colonnes `date_ouverture`/`date_fermeture`
  (nullables). `actif` reste le coupe-circuit ; `actifLe(x, iso)` = `actif ET
  iso∈[ouverture, fermeture]`. Évalué **à aujourd'hui** (v1, pas colonne-par-colonne)
  → une ligne/poste dont la fermeture est atteinte disparaît du **Planning, Placement,
  Ordonnancement** ET des **Bilans** (couverture, anticipation, projection,
  assez-competences filtrent au JOUR calculé ; feuille-route au début du rapport ;
  polyvalence et Cockpit à aujourd'hui). UI : dates
  Ouvre/Ferme + badge sur la ligne, colonne « Ferme le » sur le poste. Lectures/
  écritures tolèrent l'absence des colonnes.
...
- **`poste.imprimable` (migration 0073, colonne « Impr. » du Référentiel)** : `true` par
  défaut. `false` = le poste **n'apparaît pas sur les feuilles de placement imprimées**
  (PDF **et** PDF CE) — pour les postes qui ne servent qu'à *construire* le planning. Le
  poste **reste pleinement visible et plaçable à l'écran** de Placement : le filtre ne
  s'applique QUE au PDF (`groupsImpr` dans `PlacementBoard`, qui retire aussi les lignes
...
- Socle : `src/lib/{permissions,roles,roles-server,current-user,current-site,site-modules,week,refdata,parametres,habilitations,horaires,supabase-server,fetch-all,numeros-rotation,password-link,placement-helpers,rotation,password,erreurs,absence,absences-periodes,calendrier,quarts,poste-quart,referentiel-validite,semaine-type,interim,noms,bilans-rapports,synthese-data,verifier-site}.ts`, `src/proxy.ts`.
  - `poste-quart.ts` : **effectif par quart** (0070). `chargerPosteQuart` + `etatQuart`
    / `effectifSurQuart` / `tourneSurQuart`, trois états (aucune ligne / « – » / N).
  - `referentiel-validite.ts` : **ouverture/fermeture datée** (0071). `actifLe(x, iso)`
    + `chargerValidites(supabase, "ligne"|"poste")`, évalué à aujourd'hui.
...
- Planning : `src/app/planning/{page,PlanningGrid,PlanningFilters,AtelierFilter,QuartSelector,ConducteurToggle,QuartBandeau}.tsx`.
  ...
  **placée** (`isPoste || isFormation || exception`).
  ⚠️ **Horaire spécifique + commentaire au survol de TOUTE la case** (2026-09-16) : `excInfo`
  (calculé dans le corps de la cellule depuis `exc[excKey(pers,iso)]`) alimente le `title`
  du `<td>` **et** du bouton `.cellbtn` — plus seulement celui de la pendule 🕐. Le title du
  bouton ne contient QUE ces infos (les mentions « glisser/cliquer/Suppr » ont été retirées) ;
  vide s'il n'y a ni horaire ni commentaire.
...
- Placement (saisie glisser-déposer, droit **`placement`**) : `src/app/placement/{page,PlacementBoard,JourNav,placement.module.css}`.
  ...
  **3 colonnes**), les rangs se répartissent en colonnes (multi-colonnes CSS pilotées par la
  variable `--cols` posée sur `.poste`, calc de la largeur de tuile). Chaque occupant sans
  numéro est rendu en **rangée à part** (indispensable : sinon un poste chargé sans numéros
  restait sur une colonne unique — bug vécu, le seuil était basé sur les seuls numéros). La
  rangée « déposer ici » n'apparaît que s'il reste une place libre.
  - **Deux PDF** (bouton `imprimer(mode)`), plan filtré sur `poste.imprimable` (cf. Modèle
    métier) via `groupsImpr` : **PDF CE** (`mode="ce"`, **A4 paysage**) = plan + colonne
    « **Absents / TP du jour** » (motifs d'absence **et** bloc **Temps partiel** ; TP calculé
    serveur `tpIds`, mêmes règles que Planning/TV) ; **PDF** (`mode="simple"`, **A3 paysage**,
    plan seul) — l'A3 est activé en posant `print-a3` sur `<body>` le temps du `window.print()`
    (page nommée `plcA3` dans globals.css) et la feuille prend les dimensions A3
    (`.printSheet[data-mode="simple"]`).
  - **Sur les deux PDF** : les **numéros de rotation s'impriment même sans personne** (ligne
    « n° · libre »), et le **commentaire du jour** (`horaire_exception.motif`, saisi via la
    pendule du Planning, chargé par `placement/page.tsx` dans la prop `commentaires`) s'affiche
    en italique à côté du nom.
  - **Mise à l'échelle mesurée** (`ajusterFeuille(a3)`) : cible de hauteur **plus courte que
    la feuille** (A4 `PAGE_H=600` < `.printSheet` 650 ; A3 `PAGE_H_A3=920` < 980) → marge
    interne qui absorbe le débordement d'impression (le rendu imprimé est plus haut que la
    mesure media screen). ⚠️ **Ne PAS ré-aligner `PAGE_H` sur la hauteur de la feuille**
    (piège vécu, deux fois) : sinon la dernière ligne des plans denses est rognée.
...
- Référentiel : `src/app/admin/referentiel/*` + `src/app/api/referentiel/route.ts`
  (colonnes **N° Rot**, **Habil. requises**, **Rempl.** (PTR/PTNR), **Impr.** (imprimable),
  **Titulaire**, **effectif par quart** et **Ferme le**).
  ⚠️ **Impr.** (0073, clé `imprimable` du patch `update-poste`) = sélecteur Oui/Non par
  poste : « Non » masque le poste sur les feuilles de placement imprimées (PDF / PDF CE)
  tout en le laissant utilisable à l'écran (cf. Modèle métier `poste.imprimable`). La
  lecture des ateliers (`page.tsx`) et l'`update-poste` de la route tolèrent l'absence de
  la colonne (repli `true`, comme les colonnes de dates 0071).
  ⚠️ **Effectif par quart** (0070, op `poste-quart` avec champ `effectif`) : chaque
  colonne de quart est un champ numérique à trois états — vide « – » (ne tourne pas),
  0 (tourne à 0), N. La colonne « Effectif » unique a été retirée. Source de vérité =
  `poste_quart` via `src/lib/poste-quart.ts` ; `poste.effectif_requis` est déprécié
  (repli). Cf. l'invariant « Modèle métier ».
  ⚠️ **Ouverture/fermeture datée** (0071, ops `update-ligne`/`update-poste` avec
  `date_ouverture`/`date_fermeture`) : dates « Ouvre le »/« Ferme le » + badge sur la
  ligne, colonne « Ferme le » sur le poste. `src/lib/referentiel-validite.ts` masque à
  la date dans Planning/Placement/Ordonnancement ET les Bilans. L'`updateTable`
  de la route tolère l'absence des colonnes de dates.
...
- Absences (module **`absences`**, écran `src/app/absences-specifiques/{page,AbsencesEditor}.tsx`)
  ...
  calendrier barré `#db2777`, page `/absences-specifiques`, entrée dans `MAIN_ORDER` **juste
  après Personnel** — 2026-09-15). Défauts reproduisant l'existant : admin + chef d'équipe (périmètre RLS) en
...
- Migrations : `supabase/migrations/0001..0074` (dernière appliquée : **0073** ;
  **`0074` À EXÉCUTER** — correctif RLS isolation multi-site : droppe les policies
  permissives orphelines `stp_select`/`stp_modify` (`semaine_type_profil`, 0028) et
  `pcr_select`/`pcr_modify` (`poste_competence_requise`, 0032) que la 0043 n'avait
  jamais droppées, et qui rendaient ces deux tables visibles cross-site (semaines
  type d'un site affichées sur un autre).
  Détail 0073 —
  `poste.imprimable` (bool, défaut true) : **le poste s'imprime-t-il sur les feuilles de
  placement** (PDF / PDF CE) ? « Non » le masque à l'impression (postes ne servant qu'à
  construire le planning) sans toucher à l'écran. Colonne « Impr. » au Référentiel ;
  filtrage PDF via `groupsImpr` (`PlacementBoard`) ; lectures/écriture tolérantes à
  l'absence de la colonne (repli `true`).
  **`0072`** = `type_contrat.avec_agence` (bool) : **drapeau « piloté par agence »** généralisant
  ...
  **`0071`** = `ligne.date_ouverture`/`date_fermeture` + `poste.date_ouverture`/`date_fermeture`
  ...
  Placement / Ordonnancement ET les Bilans (par jour pour couverture/anticipation/
  projection/assez-competences, au début du rapport pour feuille-route, à aujourd'hui
  pour polyvalence/Cockpit). UI : dates Ouvre/Ferme + badge
  sur la ligne, colonne « Ferme le » sur le poste.
  **`0070`** = `poste_quart.effectif_requis` (int nullable) : **effectif par quart**.
  L'effectif est porté par (poste × quart) via le helper `src/lib/poste-quart.ts`,
  trois états (aucune ligne = repli sur `poste.effectif_requis` déprécié ; `actif=false`
  = « – » ne tourne pas ; `actif=true` = tourne à 0 ou N). Reprise : une ligne active
  par (poste × quart) actif avec l'effectif courant. Référentiel : champ numérique par
  quart, colonne « Effectif » unique retirée. Cf. l'invariant en « Modèle métier ».
  **`0069`** = `ligne.regroupement` + `personne.regroupement` (texte nullable) :
  **regroupement de lignes**, étiquette de reporting saisie inline au Référentiel et posée
  en affectation via la colonne « Service » du Personnel ; exploitée en sous-totaux par le
  bilan **Projection — feuille de route** uniquement. Pas un niveau de hiérarchie.
